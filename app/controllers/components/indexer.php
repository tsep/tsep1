...
class IndexerComponent extends Object {
    ...
    private function _run () {
        ...
        if(!$this->_singular()) {
            ...
            $this->log('Checking for jobs', LOG_INFO);
            ...
            if($this->Queue->isJob('indexer')) {
                ...
                $this->log('No jobs found; aborting', LOG_INFO);
    ...
    private function _index($job) {
        ...
        $this->log('#0003 Initializing', LOG_INFO);
        ...
        if (!empty($job['params'])) {
            ...
            //We are initializing
            $this->log('Loading dependancies', LOG_INFO);
    ...
    function processRequest ($auth_key) {
        ...
        if (!$this->_verifyAuth($auth_key)) {
            ...
            $this->log('Authentication Failed', LOG_INFO);

            return false;
        }
        else {
            $this->log('Authentication Success', LOG_INFO);
    ...
    function submitRequest ($id) {
        ...
        $this->log('#0002 Request submitted', LOG_INFO);
