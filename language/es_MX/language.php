...
<@>Haga click en este link para abrir la página
_P;
...
Define donde se encuentra TSEP de manera absoluta, es decir, si el archivo search.php esta en /home/wwww/php/tsepsearch/search.php, la direccion absoluta seria /home/www/php/tsepsearch
_P;

$tsep_lng['config_check_file_exists'] = <<<_P
<@>¿Desea comprobar si realmente existe el archivo antes de mostrar los resultados? Si elije 'true', el proceso de búsqueda es un poco mas lento, pero sólo se mostrarán archivos existentes en los resultados. Esto solo funciona si 'allow_url_open' se encuentra activado en 'php.ini'. Es probable que deba establecer esto como 'false'.
_P;

$tsep_lng['config_Color_1'] = <<<_P
<@>Primer color alternante (de una línea) - utilizado para listas largas
_P;

$tsep_lng['config_Color_2'] = <<<_P
<@>Segundo color alternante (de una línea) - utilizado para listas largas
_P;

$tsep_lng['config_Date_Style'] = <<<_P
<@>Formato de la fecha (código PHP: puede usar D, l, M & F). Se mostrará en el idioma que haya elegido. Por ejemplo: formato inglés: 'I, F d Y h:i a', o  formato alemán: 'I, d. F Y, G:i
_P;
...
<@>Esta es la ruta de la carpeta que va a ser excluída del directorio raíz de su sitio. Por ejemplo: /folder1/folder3, si http://www.yourdomain.com/folder1/folder3 contiene los directorios y archivos que van a ser excluídos. Agregue multiples entradas añadiendo ',' pero no ', '(es decir, sin espacio) <@> Esta es la dirección del directorio a ser excluída desde la raiz de su sitio. Es decir, si vamos a excluir a los contenidos de www.tusitio.com/folder1/folder3, la dirección seria /folder1/folder3. Para agregar multiples directorios, utilize comas para separar las entradas. (No utilize espacios).
_P;

$tsep_lng['config_Display_Boolean_Search'] = <<<_P
<@>Al buscar, se comprueba la versiòn de su demonio MySQL (ya que la versiòn 4.0.1 o superior es nesaria para utilizar bùsquedas con valores boleanos.) Es decir, en versiones superiores a la 4.0.0 se podràn utilizar valores boleanos. En versiones anteriores, todas las palabras que el usuario introduzca deberàn ser encontradas en una página. Solo de esta manera se mostrarà la pàgina en la lista de resultados. ¿Desea advertir al usuario si se detecta una versiòn de MySQL que no soporte el sistema de bùsqueda boleana? Recomendamos que deje esto en ´true´ - de otra manera, el usuario podrìa confundirse al no ver resultados.
_P;
...
<@>El indexer genera ìndices de archivos, pero solo de los archivos cuya extensiòn se encuentra en esta lista. (Utilize una lista separada por comas, por ejemplo, html,php,htm)
_P;

$tsep_lng['config_file_exclude'] = <<<_P
<@>Tecleè los archivos que se excluirán
_P;

$tsep_lng['config_file_exclude_help'] = <<<_P
<@>Esta es la ruta del(os) archivo(s) que va(n) a ser excluido(s). Esto es relativo al  directorio raíz de su sitio. Por ejemplo, agrege: '/folder1/folder4/login.php', si http://www.yoursite.com/folder1/folder4/login.php va a ser excluído. Agregue multiples entradas añadiendo ',' pero no ', '(es decir, sin espacios) 
_P;

$tsep_lng['config_fnExternalPhp'] = <<<_P
<@>Tecleè el nombre completo de una archivo .php de una fuente de datos externa.
_P;

$tsep_lng['config_fnExternalPhp_help'] = <<<_P
<@>Este es el nombre de un archivo (de tipo .php), afuera de TSEP, que proveè una lista de archivos de los cuales se generarà un ìndice. Ejemplo: crawler/spider.php - Vea la documentaciòn para mas detalles.
_P;
...
<@>Tiempo máximo permitido de ejecuciòn del generador de ìndices (en minutos)
_P;

$tsep_lng['config_max_exec_time_help'] = <<<_P
<@> El generador de ìndices se ejecutarà hasta el nùmero de minutos que especifique aquì. Si el generador no ha acabado cuando el tiempo haya terminado, la ejecuciòn se abortarà y se generarà un error fatal de PHP. <br /> Cabe notar que esta opciòn es afectada por la opciòn de "safe_mode" en su archivo php.ini. ¡Si safe_mode se encuentra activado, esta opciòn <u> no funcionara </u>! Si safe_mode se encuentra activo, usted será notificado en el momento que el generador comienze a trabajar.
_P;

$tsep_lng['config_how_many_hints'] = <<<_P
<@>¿Cuantas sugerencias de búsqueda seran mostradas (0 = ninguna) ?
_P;

$tsep_lng['config_show_nr_hits'] = <<<_P
<@> ¿Desea que las sugerencias de bùsqueda muestren el nùmero de pàginas que una bùsqueda regreso?
_P;

$tsep_lng['config_show_popular'] = <<<_P
<@> ¿Desea que las sugerencias de búsqueda muestren que tan popular es la bùsqueda ejecutada?
_P;

$tsep_lng['config_calc_hits_method'] = <<<_P
<@> Elija el modo en el cual se calcularàn el nùmero de pàginas de resultados
_P;

$tsep_lng['config_calc_hits_method_help'] = <<<_P
Cuando se guarde en bítacora un query de busqueda: <br /> 1 = Utilizar los resultados del query anterior<br /> 2 = Calcular la media de todos los queries<br /> 3 = Calcular  la mínima de todos los queries<br /> Calcular la máxima de todos los queries
_P;
...
El generador de índices ya esta corriendo (o algo falló.) <br /> Por favor, espera %d minutos y luego intenta de nuevo.
_P;

$tsep_lng['warning_php_safe_mode_on'] = <<<_P
<b> Advertencia: PHP esta corriendo con safe_mode</b><br /> El tiempo máximo de ejecución que especifiques en la pantalla de configuración no funcionara en el sistema.<br /> PHP fue configurado por tu administrador para abortar la ejecución despues de %d minutos.
_P;
...
<@>Términos de búsqueda utilizados previamente
_P;
...
<@>Número de veces que el termino se ha utilizado (popularidad)
_P;
...
<@>Número de páginas encontradas con el término
_P;

$tsep_lng['error_index_nothing'] = <<<_P
<@>Vacio (es decir, nada que indexar): 
_P;

$tsep_lng['error_empty_files'] = <<<_P
<@>los archivos estan vacios (nada que indexar)
_P;

$tsep_lng['display_ranking'] = <<<_P
<@>Mostrar el ranking del archivo con símbolos
_P;

$tsep_lng['ranking_100'] = <<<_P
<@>Cuántos símbolos representan 100%?
_P;

$tsep_lng['ranking_alt'] = <<<_P
<@>Teclee un texto alternativo para la imagen
_P;
...
<@>Cuando le gustaría mostrar los simbolos<br />Por favor teclee el borde (en porcentajes)
_P;

$tsep_lng['ranking_range'] = <<<_P
<@>Escoje un paso para mostrar en porcentajes
_P;
